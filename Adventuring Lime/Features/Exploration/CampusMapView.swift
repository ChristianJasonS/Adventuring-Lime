...
struct CampusMapView: View {
    ...
    // DEV 3: Recommendation State
    @State private var recParsedCount: Int = 0
    
    private let stepSize = 0.00015
...
struct GreyedMapView: UIViewRepresentable {
    ...
    @Binding var recParsedCount: Int
    ...
    private let cameraZoomSpan = MKCoordinateSpan(latitudeDelta: 0.004, longitudeDelta: 0.004)
    ...
    class Coordinator: NSObject, MKMapViewDelegate {
                ...
                for (_, line) in lines.enumerated() {
                    ...
                    if parts.count >= 2 {
                        ...
                        ann.coordinate = CLLocationCoordinate2D(
                            latitude: self.parent.campusCenter.latitude + offsetLat,
                            longitude: self.parent.campusCenter.longitude + offsetLon
                        )
                        ann.title = parts[0]
                        ann.subtitle = "Recommended for you"
                        ann.rowData = parts
                        self.mapView?.addAnnotation(ann)
                        count += 1
                    }
                }
                self.parent.recParsedCount = count
            }
        }
        ...
        func refreshOverlays() {
            ...
            if let path = currentPathOverlay { mapView.addOverlay(path, level: .aboveLabels) }
        }
        ...
        func updatePath(with coord: CLLocationCoordinate2D, showPath: Bool) {
            ...
            if !exploredMiniTiles.contains(miniTileId) {
                ...
                Task { @MainActor in
                    ...
                    if self.tileStepCounter % 5 == 0 {
                        GameManager.shared.discoverPOI(id: "demo_poi_\(UUID().uuidString)")
                    }
                }
                for (index, poly) in largeGridPolygons.enumerated() {
        ...
        func mapView(_ mapView: MKMapView, rendererFor overlay: MKOverlay) -> MKOverlayRenderer {
            if let poly = overlay as? MKPolygon {
                ...
                if poly.title == "blackout" { renderer.fillColor = .black }
                else if poly.title == "campusRed" { renderer.fillColor = UIColor.systemRed.withAlphaComponent(0.25) }
                else if poly.title == "unlockedTile" { renderer.fillColor = UIColor.systemBlue.withAlphaComponent(0.4) }
                return renderer
